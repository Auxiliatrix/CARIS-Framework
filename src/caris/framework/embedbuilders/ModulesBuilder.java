		...
		for( Handler h : Brain.modules.values() ) {
			if( !h.isRoot() ) {
				if( h.disabledOn(guild.getLongID()) ) {
					String name = StringUtilities.trim(h.name, EmbedBuilder.DESCRIPTION_CONTENT_LIMIT - 4, true);
					if( disabled.length() + name.length() > EmbedBuilder.DESCRIPTION_CONTENT_LIMIT - 4 ) {
